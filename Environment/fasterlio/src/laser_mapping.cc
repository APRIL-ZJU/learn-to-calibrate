...
void LaserMapping::setExtr(const Eigen::Vector3d &ext, const Eigen::Quaterniond &exr_qua) {
    common::V3D lidar_T_wrt_IMU;
    common::M3D lidar_R_wrt_IMU;
    lidar_T_wrt_IMU = ext;
    lidar_R_wrt_IMU = exr_qua.toRotationMatrix();

    p_imu_->SetExtrinsic(lidar_T_wrt_IMU, lidar_R_wrt_IMU);
}

std::pair<Eigen::Vector3d, Eigen::Matrix3d> LaserMapping::getExtr() {
    return std::make_pair(p_imu_->Lidar_T_wrt_IMU_, p_imu_->Lidar_R_wrt_IMU_);
}
...
void LaserMapping::Run(std::vector<double> &estTimeStamps, std::vector<double> &estXYZ, bool use_imu = false) {
    if (!SyncPackages()) {
        return;
    }
    ...
    estTimeStamps.emplace_back(msg_body_pose_.header.stamp.toSec());
    if (use_imu) {
        estXYZ.emplace_back(msg_body_pose_.pose.position.x);
        estXYZ.emplace_back(msg_body_pose_.pose.position.y);
        estXYZ.emplace_back(msg_body_pose_.pose.position.z);
    } else {
        Eigen::Quaterniond q_i(msg_body_pose_.pose.orientation.w, msg_body_pose_.pose.orientation.x,
                               msg_body_pose_.pose.orientation.y, msg_body_pose_.pose.orientation.z);
        Eigen::Vector3d t_i(msg_body_pose_.pose.position.x, msg_body_pose_.pose.position.y,
                            msg_body_pose_.pose.position.z);
        Eigen::Vector3d t_l = q_i * state_point_.offset_T_L_I + t_i;
        estXYZ.emplace_back(t_l[0]);
        estXYZ.emplace_back(t_l[1]);
        estXYZ.emplace_back(t_l[2]);
    }
...
void LaserMapping::GPSCallBack(const sensor_msgs::NavSatFix::ConstPtr &msg_in) { gps_buffer.push_back(msg_in); }
...
void LaserMapping::Savetrajectory(const std::string &traj_file) {
    auto found = traj_file.find_last_of("/");
    auto path = traj_file.substr(0, found);
    auto lidar_file = path + "/lidar_" + traj_file.substr(found + 1);
    std::ofstream ofs;
    std::cout << "saving imu traj to " << traj_file << std::endl;
    ...
    ofs.close();
    std::cout << "saving lidar traj to " << lidar_file << std::endl;
    ofs.open(lidar_file, std::ios::out);
    if (!ofs.is_open()) {
        LOG(ERROR) << "Failed to open lidar_file: " << lidar_file;
        return;
    }

    ofs << "#timestamp x y z q_x q_y q_z q_w" << std::endl;
    for (const auto &p : path_.poses) {
        Eigen::Quaterniond q_i(p.pose.orientation.w, p.pose.orientation.x, p.pose.orientation.y, p.pose.orientation.z);
        Eigen::Quaterniond q_l = q_i * state_point_.offset_R_L_I;
        Eigen::Vector3d t_i(p.pose.position.x, p.pose.position.y, p.pose.position.z);
        Eigen::Vector3d t_l = q_i * state_point_.offset_T_L_I + t_i;
        ofs << std::fixed << std::setprecision(6) << p.header.stamp.toSec() << " " << std::setprecision(15) << t_l[0]
            << " " << t_l[1] << " " << t_l[2] << " " << q_l.coeffs().x() << " " << q_l.coeffs().y() << " "
            << q_l.coeffs().z() << " " << q_l.coeffs().w() << std::endl;
    }
