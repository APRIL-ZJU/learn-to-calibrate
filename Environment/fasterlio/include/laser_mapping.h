...
class LaserMapping {
   public:
    ...
    void Run(std::vector<double>&, std::vector<double>&, bool);
    void Run();
    void setExtr(const Eigen::Vector3d& ext, const Eigen::Quaterniond& exr_qua);
    std::pair<Eigen::Vector3d, Eigen::Matrix3d> getExtr();
    // callbacks of lidar and imu
    void StandardPCLCallBack(const sensor_msgs::PointCloud2::ConstPtr &msg);
    void LivoxPCLCallBack(const livox_ros_driver::CustomMsg::ConstPtr &msg);
    void IMUCallBack(const sensor_msgs::Imu::ConstPtr &msg_in);
    void GPSCallBack(const sensor_msgs::NavSatFixConstPtr& msg_in);
    // sync lidar with imu
    bool SyncPackages();
   ...
   private:
    ...
    std::vector<sensor_msgs::NavSatFix::ConstPtr> gps_buffer;
